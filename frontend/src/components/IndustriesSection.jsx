...
import { Building2, ShoppingBag, Car, HeartPulse, ShoppingCart, Building, Cpu, Factory, ArrowRight } from 'lucide-react';
...
const IndustriesSection = () => {
  ...
  return (
    <section id="industries" className="py-24 bg-emerald-900 relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-emerald-100/80 max-w-2xl mx-auto">
            MARC combines strategy management consulting and market research to help businesses act with clarity and confidence across diverse sectors.
          </p>
        </div>
              ...
              >
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-4 transition-all duration-300 ${
                  ...
                  <Icon className={`w-7 h-7 transition-colors ${
                    activeIndustry === index ? 'text-emerald-600' : 'text-emerald-300'
                  }`} />
                </div>
                <h3 className={`font-semibold text-lg mb-2 ${
                  activeIndustry === index ? 'text-gray-900' : 'text-white'
                }`}>
                  {industry}
                </h3>
                <div className={`flex items-center gap-1 text-sm font-medium transition-opacity ${
                  activeIndustry === index ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                }`}>
                  <span className={activeIndustry === index ? 'text-emerald-600' : 'text-emerald-300'}>Learn more</span>
